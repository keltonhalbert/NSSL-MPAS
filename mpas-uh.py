...
def make_plot(filename, count):
    ...
    uhel = np.compress(mask, data.variables["updraft_helicity_max"][0])
    ...
    cm2 = ax.tricontourf(triang, uhel, levels=[50, 100, 150, 200], cmap=plt.get_cmap("magma"), extend="max", alpha=0.75)
    
    units = "m^2/s^2"
    
    plt.title("MPAS CONUS 3km Mesh\n1km AGL Reflectivity & Max Updraft Helicity\n" + time)

    plt.tight_layout()

    plt.savefig("/glade/u/home/khalbert/scripts/OUN_uhel" + str(count).zfill(3) + ".png", pad_inches=0.1, bbox_inches="tight")
    plt.clf()
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    datas = sorted(glob.glob(dirs[-1] + "/diag*"))
