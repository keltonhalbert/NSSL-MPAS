...
def make_plot(filename1, filename2, filename3, count):
    ...
    wind1 = np.compress(mask, data1.variables["wind_speed_level1_max"][0]) 
    wind2 = np.compress(mask, data2.variables["wind_speed_level1_max"][0])
    wind3 = np.compress(mask, data3.variables["wind_speed_level1_max"][0])
    ...
    cm1 = ax.tricontourf(triang, wind3, levels=np.arange(25,27,1), cmap=plt.get_cmap("Purples"), extend="max")
    cm2 = ax.tricontourf(triang, wind2, levels=np.arange(25,27,1), cmap=plt.get_cmap("Greens"), extend="max")
    cm3 = ax.tricontourf(triang, wind1, levels=np.arange(25,27,1), cmap=plt.get_cmap("Reds"), extend="max")
    
    plt.tight_layout()

    plt.savefig("/glade/u/home/khalbert/scripts/OUN_paint_wind" + str(count).zfill(3) + ".png", pad_inches=0.1, bbox_inches="tight")
    plt.clf()
    plt.close(fig)
