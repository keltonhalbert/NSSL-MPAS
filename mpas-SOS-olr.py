...
def make_plot(filename, count):
    ...
    cape = np.compress(mask, data.variables["olrtoa"][0]) 
    ...
    cm = ax.tricontourf(triang, cape, levels=np.arange(0,400,20), cmap=plt.get_cmap("bone_r"), extend="max")
    ...
    plt.savefig("/glade/u/home/khalbert/scripts/SOS-olr" + str(count).zfill(3) + ".png", pad_inches=0, bbox_inches="tight", dpi=75)
    plt.clf()
    plt.close(fig)
