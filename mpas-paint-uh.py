...
def make_plot(filename1, filename2, filename3, count):
    ...
    uh1 = np.compress(mask, data1.variables["updraft_helicity_max"][0]) 
    uh2 = np.compress(mask, data2.variables["updraft_helicity_max"][0])
    uh3 = np.compress(mask, data3.variables["updraft_helicity_max"][0])
    ...
    cm1 = ax.tricontourf(triang, uh3, levels=np.arange(25,80,25), cmap=plt.get_cmap("Purples"), extend="max")
    cm2 = ax.tricontourf(triang, uh2, levels=np.arange(25,80,25), cmap=plt.get_cmap("Greens"), extend="max")
    cm3 = ax.tricontourf(triang, uh1, levels=np.arange(25,80,25), cmap=plt.get_cmap("Reds"), extend="max")
    
    plt.tight_layout()

    plt.savefig("/glade/u/home/khalbert/scripts/OUN_paint_uh" + str(count).zfill(3) + ".png", pad_inches=0.1, bbox_inches="tight")
    plt.clf()
    plt.close(fig)
